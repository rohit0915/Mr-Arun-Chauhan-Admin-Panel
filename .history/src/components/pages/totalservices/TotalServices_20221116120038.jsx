...
const TotalServices = () => {
  ...
  const deleteService = async (id) => {
    try {
      const data = await axios.delete(
        ` https://pitzone-backend.herokuapp.com/api/service-type/${id}`
      );
  ...
  // Add Service
  const [serviceTypeName, setS] = useState("");
  ...
  const editService = async (e) => {
    try{
      const {data } = await axios.put(` https://pitzone-backend.herokuapp.com/api/service-type/${id}`)
      ...
      <section>
        ...
        >
          <Box sx={style}>
            ...
            <form className="mt-14" onSubmit={addService}>
