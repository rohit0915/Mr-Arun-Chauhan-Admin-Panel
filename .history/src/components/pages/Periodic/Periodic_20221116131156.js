...
const Periodic = () => {
    ...
    //All Services
    const periodicService = async () => {
      try {
        const { data } = await axios.get(
          "https://pitzone-backend.herokuapp.com/api/perodic-service"
        );
        setData(data);
      } catch (err) {
        console.log("err", err);
                    ...
                    <tr key={i} className="tracking-wider text-gray-900 ">
                      <td className=" px-4 py-3  space-x-3 ">
                        <img
                          ...
                          src={
                            e.periodicServiceImg.url
                              ? e.periodicServiceImg.url
                              : "https://www.learnaboutlogistics.com/wp-content/uploads/2016/07/photodune-6221194-customer-service-team-s-e1468394369939-742x353.jpg"
                          }
                          style={{ width: "80px" }}
                          className="img-fluid thumbnail rounded"
                        />
                      </td>
                      <td className=" px-4 py-3  space-x-3 ">{e.title}</td>
                      <td className=" px-4 py-3  space-x-3 ">{e.price}</td>
                      <td className=" px-4 py-3  space-x-3 ">{e.realPrice}</td>
                      <td className=" px-4 py-3  space-x-3 ">{e.pickupCharge}</td>
                      <td className=" px-4 py-3  space-x-3 ">{e.time}</td>
                      <td className=" px-4 py-3  space-x-3 ">{e.ratings}</td>
                      <td className=" px-4 py-3  space-x-3 ">{e.numOfReviews}</td>
